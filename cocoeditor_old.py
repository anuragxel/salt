...
dataset_explorer = DatasetExplorer("dataset", categories= ["generic_object"], coco_json_path="dataset/annotations.json")
...
def on_mouse(x, y, label):
    ...
    if len(input_point) == 0:
        ...
        input_point = np.vstack([
            input_point, 
            np.array([x, y])
        ])
    input_label = np.append(input_label, label)
    masks, low_res_logits = onnx_helper.call(image, image_embedding, input_point, input_label, low_res_logits=low_res_logits)
    display = copy.deepcopy(image_bgr)
    anns, colors = dataset_explorer.get_annotations(image_id, return_colors=True)
    display = draw_annotations(display, anns, colors)
